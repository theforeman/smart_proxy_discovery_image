...
module Proxy::DiscoveryImage
  class PowerApi < ::Sinatra::Base
    ...
    put "/kexec" do
      ...
      system("chvt", "2")
      is_secureboot = system("grep", "1", "/sys/kernel/security/securelevel")
      ...
      if is_secureboot
       run_after_response 2, kexec, "-s", "--debug", "--force", "--reset-vga", "--append=#{data['append']}", "--initrd=/tmp/initrd.img", "/tmp/vmlinuz", *data['extra']
        else
